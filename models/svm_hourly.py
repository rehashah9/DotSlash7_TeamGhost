...
data = pd.read_csv('aws_ec2_instance_metric_cpu_utilization_hourly.csv')
data['timestamp'] = pd.to_datetime(data['timestamp'])  
data.set_index('timestamp', inplace=True)

data = data[['maximum']].resample('h').mean().fillna(method='ffill')
...
joblib.dump(svm_model, 'svm_hourly.pkl')

# Load the saved SVM model
loaded_svm_model = joblib.load('svm_hourly.pkl')